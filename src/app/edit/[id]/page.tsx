...
export default function EditGistPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (coverFile) {
      const r = await uploadBytes(ref(storage, `covers/${coverFile.name}-${Date.now()}`), coverFile);
      coverUrl = await getDownloadURL(r.ref);
    }
        ...
        <div>
          ...
          {blocks.map((blk, i) => (
            <div key={i} className="space-y-2 mb-4 border p-3 rounded bg-gray-50">
              <input
                type="text" placeholder="Block title"
